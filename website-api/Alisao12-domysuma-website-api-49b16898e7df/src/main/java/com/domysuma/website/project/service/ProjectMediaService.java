...
import com.domysuma.website.core.service.StorageService;
...
public class ProjectMediaService {
    ...
    public void createProjectMedia(Project project, Collection<ProjectMediaCreate> mediaCreate) {
        ...
        mediaCreate.forEach(media -> {
            ...
            try {
                ...
                // Get the full public URL and save it
                String fullUrl = storageService.getPublicUrl(filePath);
                projectMedia.setPath(fullUrl);
                
                log.info("Project media uploaded - File path: {}, Full URL: {}", filePath, fullUrl);
                
            } catch (IOException e) {
                log.error("Failed to upload project media: {}", e.getMessage());
                throw APIException.internalError("Failed to upload project media");
            }
    ...
    public void removeProjectMedia(Long designMediaId) {
        ...
        // Extract just the path from the full URL before deleting
        String path = designMedia.getPath();
        if (path.contains("digitaloceanspaces.com/")) {
            path = path.substring(path.indexOf("digitaloceanspaces.com/") + 23);
        }
